...
##前端服务
1. 简介：
我们的前端服务和传统意义上所谓的前端和前端ui略有不同，我们的前端服务不仅包括UI部分还有js 和html的部分，同时也包括它的server部分。
这样的话有前台的html、js和css加上后端的go的web server共同组成了一套真正的前端，就是大前端。为什么要这么做呢？
一般的小的网站或者小的个人站点都是直接一个web server把结果返回给前端，但是在这些逻辑稍微有些复杂的网站中，这样做有很多弊端。
第一个它没有办法解耦，第二效率低，第三是它能承载的业务量是很有限的，而且扩容起来非常复杂，而且无法进行分布式化的拆分和解耦。

2. Go的模板引擎
 * 模板引擎是将html解析和元素预置替换生成最终页面的工具。在html中嵌入它的元素，然后让模板引擎把它的元素和它的简单的语言进行置换，最后展现在外面给用户看的部分
 * Go 的模板有两种text/template和html/template：text/是非常复杂的，这里介绍html/template
 * Go 的html/template采用的是 动态生成的模式。把你想要的生成的东西，或者想要替换的字段在html中用我们的模板格式给它标出来，然后
 request进来的时候go的web server会拿到它的模板，然后进行业务处理，将它替换，然后再randder 渲染成真正的前端的页面

3. 文件创建：
* web：main.go defs.go handlers.go + client.go(就等于是我们的一个代理过程)
> //再多说两句：我们在这边的大前端它等于是一个很薄的一层，这一层只做两件事情：1.转发业务的请求2.把它用来模板化的东西真正的通过网页渲染到前端
  //转发业务请求有两中模式：proxy模式和api模式   
* template:需要添加两个文件image(用来放一些静态图片)和scripts(用来放js) ,还需要其他文件，向首页(home.html、user_home.html)

4.使用template实现静态页面的渲染
